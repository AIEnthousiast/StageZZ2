...
class GraphInstance:
    def __init__(self,edges=None,blockages=None,generate_graph=True) :
        self.edges = None
        self.blockages = None
        if edges != None:
            self.edges = list(set(edges.keys()))
    ...
    def RDFS(self):
        ...
        def adjw(s):
            ...
            return sorted(neighbours,key=lambda x : w[self.inv_map[x]])

        def DFS_visit(i,f):
            color[self.inv_map[i]] = -1

            for j in adjw(i):
                if color[self.inv_map[j]] == 1:
                    A.append((i,j))
                    DFS_visit(j,i)
                elif j != f and color[self.inv_map[j]] != 0:
                    A.append((i,j))
            color[self.inv_map[i]] = 0
    ...
    def get_neighbours_out(self,s):
        return [n for (i,n) in self.edges if i == s]

    def get_neighbours_in(self,s):
        return [n for (n,i) in self.edges if i == s]

    def dfs(self,visited,node):  #function for dfs 
        if node not in visited:
            visited.append(node)
            for neighbour in self.get_neighbours_out(node):
    ...
    def kosaraju(self):
        visited = []
        s = self.nodes[0]
        A = [(i,j) for (i,j) in self.edges if (i,j) not in self.blockages]  + [(j,i) for (i,j) in self.edges if (j,i) not in self.blockages] + [(j,i) for (i,j) in self.blockages]
        dfs(visited,A,s)
        if len(visited) == self.number_of_nodes:
            reverse_edges = [(j,i) for (i,j) in A]
            visited = []
            dfs(visited,reverse_edges,s)
    ...
    def generate_blockages(self,n_blockages):
        self.blockages = []
        n = 0
        temp_edges = self.edges[:] + [(j,i) for (i,j) in self.edges]
        blocks = []
        it = 0
        while n != n_blockages and it < 100000:
            r = random.choice(temp_edges)
            temp_edges.remove(r)
            if kosaraju(self.nodes,temp_edges):
                blocks.append(r)
                n += 1
            else:
                temp_edges.append(r)
            it += 1
        self.blockages = blocks
...
class GridInstance(GraphInstance):
    def __init__(self,n,generate_graph=True):
        super().__init__(edges=GridInstance.create_grid_instance_edges(n),generate_graph=generate_graph)
        self.n = n
    ...
    def create_grid_instance_edges(cls,n):
        ...
        for i in range(n):
            for j in range(n):
                if i < n-1:
                    edges[(i*n+j+1,(i+1)*n+j+1)] = 1
                if j < n-1:
                    edges[(i*n+j+1, i*n+j+2)] = 1
        return edges
    ...
    def create_instance_with_blockages(cls,n,n_blockages):
        ...
        for i in range(n_blockages):
            bridges = gI.bridges()
            if bridges:
                rem_edge = random.choice([e for e in gI.edges if e not in bridges])
            else:
                rem_edge = random.choice(gI.edges)
            gI.edges.remove(rem_edge)
            blockages.append(rem_edge)
            gI.generate_graph()
            
        gI.edges = edges
        gI.blockages = blockages
        gI.generate_graph()
...
def get_neighbours_out(edges,s):
    return [n for (i,n) in edges if i == s]

def get_neighbours_in(edges,s):
    return [n for (n,i) in edges if i == s]
